...
class TestConverter(unittest.TestCase):
    def test_converters_gts(self):
        ...
        self.assertTrue( os.path.isdir(gts_dir))
        ...
        self.assertTrue( len(coco_bbs_v1) == len(coco_bbs_v2) == len(cvat_bbs) == len(imagenet_bbs) == len(
        ...
        for coco_bb_v1, coco_bb_v2, cvat_bb, imagenet_bb, labelme_bb, openimage_bb, vocpascal_bb, yolo_bb in zip(
            ...
            self.assertTrue( coco_bb_v1 == coco_bb_v2 == cvat_bb == imagenet_bb == labelme_bb == openimage_bb == vocpascal_bb == yolo_bb)


    def test_converters_dets(self):
        # Defining paths with images and annotations
        images_dir = 'data/database/images'
        ...
        for coco_bb_v1, coco_bb_v2, cvat_bb, imagenet_bb, labelme_bb, openimage_bb, vocpascal_bb, yolo_bb in zip(
                coco_bbs_v1, coco_bbs_v2, cvat_bbs, imagenet_bbs, labelme_bbs, openimage_bbs,
                vocpascal_bbs, yolo_bbs):
            self.assertTrue( coco_bb_v1 == cvat_bb == imagenet_bb == labelme_bb == openimage_bb == vocpascal_bb == yolo_bb)


    def test_toy_example_dets(self):
        dir_annots_dets = 'toyexample/dets_classid_rel_xcycwh'

        yolo_files = general_utils.get_files_recursively(dir_annots_dets)
        self.assertTrue( len(yolo_files) > 0)
        for pascal_file in yolo_files:
            self.assertTrue( validations.is_yolo_format(pascal_file, bb_types=[BBType.DETECTED]))


    def test_toy_example_dets_compatibility(self):
    ...
    def test_toy_example_gts(self):
        ...
        for f in files:
            self.assertTrue( validations.is_pascal_format(
        ...
        self.assertTrue( len(files) > 0)
        for f in files:
            self.assertTrue( validations.is_coco_format(f), 'File {f} does not follow the expected format (COCO)')
        ...
        self.assertTrue( len(coco_bbs) == len(pascal_bbs))

        for coco_bb, pascal_bb in zip(coco_bbs, pascal_bbs):
            self.assertTrue( coco_bb == pascal_bb)
